...
public class ModelsTests
{
    ...
    public void HandlerDiscoveryInfo_PropertiesAndEquality_WorkAsExpected()
    {
        ...
        unchecked
        {
            var expected = StringComparer.Ordinal.GetHashCode("Sample.MyHandler");
            expected = (expected * 397) ^ StringComparer.Ordinal.GetHashCode("Sample.MyMessage");
            expected = (expected * 397) ^ StringComparer.Ordinal.GetHashCode("my-msg");
            expected = (expected * 397) ^ StringComparer.Ordinal.GetHashCode("MyHandler");
            info1.GetHashCode().Should().Be(expected);
        }
    ...
    public void PartitionKeyInfo_PropertiesAndEquality_WorkAsExpected()
    {
        ...
        unchecked
        {
            var expected = StringComparer.Ordinal.GetHashCode("Sample.OrderMessage");
            expected = (expected * 397) ^ StringComparer.Ordinal.GetHashCode("OrderId");
            expected = (expected * 397) ^ StringComparer.Ordinal.GetHashCode("System.Guid");
            info1.GetHashCode().Should().Be(expected);
        }
