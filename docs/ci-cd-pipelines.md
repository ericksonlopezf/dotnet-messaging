...
This document defines the complete Continuous Integration and Continuous Deployment (CI/CD) architecture, quality gates, and automated validation workflows for `EricksonLopez.Messaging`.
...
Validates that `EricksonLopez.Messaging` packages genuinely compile and run under Native AOT:

1. Publishes `tests/EricksonLopez.Messaging.AotSmokeTest/` with `--PublishAot=true` and `-p:TreatWarningsAsErrors=true`.
